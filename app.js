document.addEventListener('DOMContentLoaded', () => {
  ...
  // Dynamically enhance color inputs with a text input for hex codes
  document.querySelectorAll('input[type="color"]').forEach(colorEl => {
    ...
    textEl.type = 'text';
    textEl.className = 'color-code-input';
    textEl.value = colorEl.value.toUpperCase();
    textEl.placeholder = colorEl.value;
    textEl.style.flex = '1';
    textEl.style.height = '35px';
    textEl.style.background = 'var(--bg-input)';
    textEl.style.border = '1px solid var(--border-color)';
    ...
    // Sync text -> picker & trigger event
  ...
  function updatePreview() {
    ...
    const cardBg = document.querySelector('.card-multiply-bg');
    const bgImgUrl = state.images.bg ? state.images.bg : (state.bgUrl ? state.bgUrl : '');
    
    if (cardBg) {
      if (bgImgUrl) {
        cardBg.style.backgroundImage = `linear-gradient(0deg, rgba(0, 0, 0, 0.70) 0%, rgba(0, 0, 0, 0.70) 100%), url('${bgImgUrl}')`;
        cardBg.style.backgroundSize = 'cover, cover';
        cardBg.style.backgroundPosition = 'center, center';
      } else {
        cardBg.style.backgroundImage = 'linear-gradient(0deg, rgba(0, 0, 0, 0.70) 0%, rgba(0, 0, 0, 0.70) 100%)';
      }
    }
    
    // Clear old fullscreen bg
    if (preview.bg) {
    ...
    // 2.1 Render Header Patterns
    const patternType = state.patternType;
    const patternFg = state.patternFgColor;
    const hasHeaderBg = !!state.images.headerBg;
    const patternBg = hasHeaderBg ? 'transparent' : state.patternBgColor;
  ...
  function populateEditor() {
    ...
    // Sync all enhanced color text inputs to match the current color picker values
    document.querySelectorAll('input[type="color"]').forEach(colorEl => {
      const textEl = colorEl.parentNode.querySelector('.color-code-input');
      if (textEl) {
        textEl.value = colorEl.value.toUpperCase();
      }
    });
  ...
  const CROP_CONFIGS = {
    ...
    bg: { aspectRatio: 1500 / 848, width: 1500, height: 848 },
  ...
  function init() {
    ...
    if (cached) {
      try {
        ...
        // Auto-migrate old default speech text to the new bold default text
        if (state.speech === '바이오나 한마디를 이곳에 적어보세요 **미키오 바보**') {
          state.speech = DEFAULT_STATE.speech;
        }
        // Auto-migrate old default space background image to empty
        if (state.bgUrl === 'https://api.builder.io/api/v1/image/assets/TEMP/73ea9d898dd91395444ffb97be38dae6bc9c120e?width=3000') {
          state.bgUrl = '';
        }
      } catch (e) {
